...
function acceptDeclineResponse(data,status) {
    ...
    if (status == 'success') {
        ...
        alert('failed to Accept or Decline Request' + status);
    }
}
function acceptDeclineRequest(event) {
    ...
    req=event.target.id.split("-")
    user_id=req[1]
    accept=req[0]
    let json_data = { 'decision':accept,'user_id':user_id};
    // globally defined in messages.djhtml using i{% url 'social:accept_decline_url' %}
